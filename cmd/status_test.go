...
func TestRunStatusUsesCommandCancellation(t *testing.T) {
	...
	s := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		t.Error("canceled status request reached server")
	}))
	defer s.Close()
	client := api.NewClient(s.URL, "user@pve!test", "secret", true)
	if err := runStatus(client); !errors.Is(err, context.Canceled) {
		t.Errorf("runStatus() error = %v, want context cancellation", err)
	}
}

func TestRunStatusCancelsInFlightFetch(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	old := rootCmd.Context()
	rootCmd.SetContext(ctx)
	...
	go func() { done <- runStatus(client) }()
	<-started
	cancel()
	select {
	case err := <-done:
		if !errors.Is(err, context.Canceled) {
			t.Errorf("runStatus() error = %v, want cancellation", err)
		}
	case <-time.After(time.Second):
		s.CloseClientConnections()
		t.Fatal("in-flight status fetch did not stop after cancellation")
	}
}
